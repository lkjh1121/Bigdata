...
# 파일 디렉터리 생성
dir = "./weathers/{:%Y-%m-%d}".format(datetime.now())
...
# 파일 생성 및 데이터 파싱
fname = "Weathers-{:%Y-%m-%d-%H-%M.csv}".format(datetime.now())
file = open(dir+'/'+fname, 'w', encoding='utf-8')

# 지역 출력
tag_trs = browser.find_elements(By.CSS_SELECTOR, '#weather_table > tbody > tr')
